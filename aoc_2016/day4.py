...
def get_code_and_checksum(data):
    chk = data.split('[')[1]
    code = data.split('[')[0]
    chk = chk.replace(']', "")

    return code, chk, int(code.split('-')[-1])


def verify(d):
    code, chk, _id = get_code_and_checksum(d)
    
    #print(f"chk -> {chk}")
    #print(f"code -> {code}")

    cw = count_words(code, chk)
    #print(f'word counting: {cw}')

    curr = -1
    last_key = ''
    for k in cw:
        #print(f"k {k} val {cw[k]} -> last_k {last_key}:val {cw[last_key] if last_key else None} ")
        ...
        if curr == cw[k] and ord(last_key) < ord(k): 
            last_key = k
            curr = cw[k]
            continue
...
def solution2(lines):
    ...
    for line in lines:
        _id = verify(line)
        if _id > 0:
            decoded, _id = decrypt_rotated_cypher(line)
            if 'north' in decoded:
                print('got it: ', decoded)
                return _id
...
import string
def decrypt_rotated_cypher(entry):
    alpha_len = len(string.ascii_lowercase)
    code, chk, _id = get_code_and_checksum(entry)
    ...
    for token in code: 
        ...
        p = string.ascii_lowercase.find(token)
        rotate = (jumps + p) % alpha_len
        w.append(string.ascii_lowercase[rotate])
...
_i = data.split("\n")
_i = list(filter(lambda n: n!="", _i))
#nzydfxpc-rclop-qwzhpc-qtylyntyr-769[oshgk]

print(f"solution 1: {solution1(_i)}")
print(f"solution 2: {solution2(_i)}")
