...
def solution1(txt, solver_fn):
    lines = filter(lambda line: line, txt.splitlines())

    digits = list(map(lambda line: line.split(' '), lines))
    digits = list(map(lambda line: list(map(lambda n: int(n), line)), digits))
